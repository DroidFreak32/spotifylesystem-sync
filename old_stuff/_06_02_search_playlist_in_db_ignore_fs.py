...
def is_item_in_db(db_tag=None, track_tag=None):
    if(isinstance(str_to_list(db_tag), list)):
        for tag in db_tag:
            if tag.casefold() in track_tag.casefold():
                return True
    if db_tag.casefold() in track_tag.casefold():
        return True
    return False
    ...
    for row in album_artist.tracks:
        if is_item_in_db(row.TITLE, track_metadata['TITLE']):
            if row.TITLE.casefold() != track_metadata['TITLE'].casefold():
                print(f"Spotify track: {track_metadata['ALBUMARTIST']} - {track_metadata['TITLE']}: {track_metadata['SPOTIFY']}\n Track in DB: {row.PATH}\n Are these the same?")
                answer = input("Y/N: ")
